...
path='./raw/190412peddleimu/black_validation.csv'

Reader.figure(path=path,header=0)
raw=Reader.export()
adp=QuickAdp(data=raw,xsens_smalerate=40)
...
index=[0]
plt.figure(2)
test.movingavg(window_s=0.5,shift_s=0.25,samplerate=40,index=[0],title=talbelist_item+'_self')
#plt.figure(3)
test.moving_avg.peak_cadence(title=talbelist_item+'_self',tag=str(index[0]))


#t1=Signal(imu1.eu.values[:,0])
#t1.peak_detection(title=talbelist_item+'_self',tag=str(index[0]))
...
plt.grid()
plt.plot(rpm,label='mintab')  
plt.legend()
#t1.peak_green(title=talbelist_item+'_self',tag=str(index[0]))
