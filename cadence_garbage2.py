...
#    print(ll)
    for cad in cadence:
        ...
        ys=np.linspace(-180,180,cad/60*samplerate)
        f_ys=np.hstack((f_ys,ys))
    f_xs=np.linspace(0,len(ys)/samplerate,len(f_ys))
    return f_xs,f_ys

xs,ys=fakewave(samplerate,cadence)
plt.plot(xs,ys)
plt.show()
from peakdetect import *
maxtab, mintab = peakdet(ys,peakdetect_threshold)


estimated_f=np.diff(maxtab[:,0])/samplerate
plt.plot(cadence[1:-1],label='input_f',linewidth=3)
print(len(cadence[:len(estimated_f)]),len(np.hstack((np.zeros(1),estimated_f))))
plt.plot(estimated_f*60,label='output_f',linewidth=3,linestyle=':')
plt.legend()
